...
class CdnServicesFilesystemAdapter implements Filesystem, Cloud
{
    ...
    public function put($path, $contents, $options = []): bool
    {
        try {
            ...
            // Build multipart: image, disk + optional caption, tags, folder, visibility
            $multipart = [
                ['name' => 'image', 'contents' => fopen($tempPath, 'r'), 'filename' => basename($path)],
                ['name' => 'disk', 'contents' => $options['disk'] ?? $this->disk],
            ];
            if (!empty($options['caption'])) {
                $multipart[] = ['name' => 'caption', 'contents' => (string) $options['caption']];
            }
            if (isset($options['tags']) && is_array($options['tags'])) {
                $multipart[] = ['name' => 'tags', 'contents' => json_encode($options['tags'])];
            } elseif (!empty($options['tags'])) {
                $multipart[] = ['name' => 'tags', 'contents' => is_string($options['tags']) ? $options['tags'] : json_encode($options['tags'])];
            }
            if (!empty($options['folder'])) {
                $multipart[] = ['name' => 'folder', 'contents' => (string) $options['folder']];
            }
            if (isset($options['visibility']) && in_array($options['visibility'], ['public', 'private', 'unlisted'], true)) {
                $multipart[] = ['name' => 'visibility', 'contents' => $options['visibility']];
            }
            $response = $this->makeRequest('post', '/api/upload', ['multipart' => $multipart]);
