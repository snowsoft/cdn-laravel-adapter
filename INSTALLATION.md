...
**Paket:** [snowsoft/cdn-laravel-adapter](https://github.com/snowsoft/cdn-laravel-adapter)
...
### 1. Composer ile kurulum (önerilen)
...
Laravel paket keşfi provider ve alias'ı otomatik yükleyecektir. Config yayınlamak için:

```bash
php artisan vendor:publish --tag=cdn-services-config
```

### 2. Manuel kurulum (paketi kopyalama)

Adapter klasörünü projenize kopyalayın:

```bash
# Örnek: repoyu klonlayıp packages altına almak
git clone https://github.com/snowsoft/cdn-laravel-adapter.git packages/cdn-laravel-adapter
```
...
```json
{
    "autoload": {
        "psr-4": {
            ...
            "CdnServices\\": "packages/cdn-laravel-adapter/src/"
        }
    }
}
```
...
### 3. Service Provider kaydı (manuel kurulumda)
...
### 4. Config dosyası (manuel kurulumda)

```bash
php artisan vendor:publish --tag=cdn-services-config
```

veya manuel: `cp packages/cdn-laravel-adapter/config/cdn-services.php config/`
...
Detaylı kullanım örnekleri için `README.md` dosyasına bakın.
